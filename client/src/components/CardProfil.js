...
import { deleteMyMatchs, getMatchs } from "../action/matchAction";
    ...
    >
      {auth.user && (
        <>
          ...
          {auth.user._id == "601b15469986e23024e94219" ? (
            <NavLink
              className="nav-link navi font-weight-bold alert-secondary btn m-3 rounded-pill border-dark w-50 m-auto "
              to="/admin"
                      ...
                      {auth.user.notif && (
                        <span className="text-center text-success">
                          {auth.user.notif.length ? (
                            <h3>
                              Vous êtes invités à rejoindre ces matchs &#128512;
                            </h3>
                          ) : (
